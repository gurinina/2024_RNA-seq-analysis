...
geomean <- function(x, na.rm = TRUE){
    ...
    # (1/length(x)) * log(prod(x)) is the same as
    # log(prod(x)^(1/length(x)))
    # sum(log(x)) is the same as log(prod(x))
    # mean(log(x), na.rm = na.rm) is the same as 
    # (1/length(x)) * sum(log(x), na.rm = na.rm)
...
Hint: use the `apply` and `sweep` functions
...
- Ans: color, fontsize_row,border_color points = +3
...
# the default behavior of the function `DESeq2::results` is to return the comparison of the last, so your condition of interest should come last:
   ~ sex + treatment + age
