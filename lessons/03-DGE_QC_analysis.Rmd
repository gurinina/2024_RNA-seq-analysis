...
solutions("The sample groups 'Ctrl' and 'Sci' separate well on PC1 and PC2")
```
...
```{r, results = "asis", echo = FALSE}
solutions("Yes, sci8_rep2, ctrl0_rep3 cluster away from their replicates")
```
...
```{r, results = "asis", echo = FALSE}
solutions("Yes, sci8 and sci16 don't separate well, the ctrl0 group shows big variations among replicates")
```
...
solutions(
  'display.brewer.all()\n
   ...
   pheatmap(rld_cor, color = heat.colors, border_color = "black", 
   fontsize = 10, fontsize_row = 12, show_rownames = T, show_colnames = T)\n
   
   "It changes the color palette, adds a border color, changes the font size 
    of the heatmap, row names, and column names, and shows the row and 
    column names."'
)
