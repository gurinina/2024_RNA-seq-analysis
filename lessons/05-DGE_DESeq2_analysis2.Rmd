...
Calculate the expected mean expression for gene \( 1 \) in sample \( 4 \) using the design matrix and beta coefficients using the design matrix is \( X_j \) and the beta coefficients are \( \beta_i \).
...
To test this, DESeq2 uses the **Wald test**, which compares the estimated log2 fold change (LFC) to its standard error.

### Wald Test for Log2 Fold Change

The **Wald test** assesses whether the log2 fold change is significantly different from zero:

1. The log2 fold change is divided by its standard error to compute a **z-statistic**.

2. The z-statistic is compared to a standard normal distribution to obtain the p-value.
...
Shrunken log2 fold changes are used in DESeq2 to provide more reliable estimates of fold changes, especially for genes with low counts or high variability. Without shrinkage, log2 fold changes for these genes can be overestimated or erratic, leading to misleading results.

By applying shrinkage, DESeq2 moderates the large, uncertain fold changes toward more realistic values, reducing noise and improving the overall interpretability of the data. This is particularly important when comparing genes with low expression levels, where the raw fold change estimates tend to be more variable and less accurate.

In summary, shrinkage helps stabilize fold change estimates, reducing false positives and allowing for more biologically meaningful conclusions.
