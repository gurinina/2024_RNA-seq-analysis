...
The`enrichGO()` function from the `clusterProfiler` package performs the over-representation analysis for the significant genes of interest (`sigOE_genes`) compared to the background gene list (`allOE_genes`), using the specified parameters.
...
`enrichGO()` returns the enrichment GO categories after FDR control.
...
The `ego` object contains information about the enriched GO terms, including the term ID, description, gene list, and statistical information. We can extract the `ego` object as a data frame for further analysis and visualization.
```{r}
...
Now we have the gene lists for each GO term, we can calculate the semantic similarity between the terms based on the shared genes and visualize them in a network. To visualize the network we need to calculate edges.
```{r}
...
Using the edges data frame, we can create a network plot to visualize the relationships between the enriched GO terms based on their semantic similarity. The nodes represent GO terms, and the edges indicate the strength of the similarity between terms. 
```{r}
...
We can also define edges based on the Jaccard similarity between the gene lists of each pair of GO terms. The Jaccard similarity is defined as the size of the intersection divided by the size of the union of the GO terms. 
```{r}
...
The resulting network plots shows the relationships between the enriched GO terms based on either Semantic or Jaccard similarity of their gene lists. The nodes represent GO terms, and the edges indicate the strength of the similarity between terms. This visualization helps identify clusters of related terms and provides insights into the functional relationships between biological processes.
...
# category labels
emapplot(pwt, showCategory = 30, node_label = "category", 
         cex_label_group = 3, 
         layout.params = list("mds")) +
         theme(legend.position = "none")

# group labels 
emapplot(pwt, showCategory = 150, node_label = "group") +
       theme(legend.position = "none")
...
ssplot(pwt, showCategory = 150, node_label = "group", 
       with_edge = FALSE, 
       group_category = TRUE,
       layout.params = list("mds")) +
       theme(legend.position = "none")
...
We first need to set up the visualization using the `visSetup` function, which prepares the data for network visualization. We then run the `runNetwork` function to generate the interactive network plot.
