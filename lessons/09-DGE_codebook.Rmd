...
# extract results table
res_tableKD_unshrunken <- results(dds, contrast = contrast_kd, 
                                  alpha = 0.05)
...
normalized_counts <- read.delim("data/normalized_counts.txt", 
                                row.names = 1)
...
If you wish to change the appearance of this plot, we can save the output of `plotCounts()` to a variable specifying the `returnData = TRUE` argument. This will save the normalized counts for the gene `MOV10` to a data frame object. We can then use ggplot2 to plot the normalized counts for `MOV10` across all samples.
...
`Pivot_longer()` takes the data frame and converts it from a wide format to a long format. The `cols` argument specifies the columns that will be pivoted, in this case, the columns containing the normalized counts. The `names_to` argument specifies the name of the column that will contain the column names of the original data frame, and the `values_to` argument specifies the name of the column that will contain the values of the original data frame.
```{r}
# Pivoting the columns to have normalized counts to a single column

pivoted_top20_sigOE <- top20_sigOE_norm %>%
  pivot_longer(cols = colnames(top20_sigOE_norm)[2:9], 
  names_to = "samplename", values_to = "normalized_counts")

## check the column header in the "pivoted" data frame
head(pivoted_top20_sigOE)
```

Now, if we want our counts colored by sample group, then we need to combine the metadata information with the melted normalized counts data into the same data frame for input to `ggplot()`. We can do this using the `inner_join()` function from the **dplyr** package.

`inner_join()` will merge 2 data frames by the colname in x that matches a column name in y. 

`inner_join(mov10_meta, pivoted_top20_sigOE)` will merge the `mov10_meta` data frame with the `pivoted_top20_sigOE` data frame by the `samplename` column.
```{r}
...
Now let’s draw the heatmap using pheatmap. We can also add annotations to the heatmap to indicate the sample type. Annotations can be added to the heatmap using the `annotation_row` or `annotation_col` argument in `pheatmap()`. The `annotation_col` argument requires a data frame with the same row names as the column names of the matrix used to generate the heatmap. The `annotation` data frame should contain the annotation information for each sample.

We can also add annotation colors to the heatmap using the `annotation_colors` argument. This argument requires a list with the annotation column name as the list name and the colors as the list values.
```{r}
...
# add a column to the res_tableOE tibble to indicate whether the gene is differentially expressed based on the thresholds we set
...
# Prepare GO data for Biological Process (BP) ontology 
# and similarity calculation
go_data <- GOSemSim::godata(annoDb="org.Hs.eg.db", ont = "BP",
           keytype = "SYMBOL", computeIC = TRUE)

# Compute semantic similarity
similarity_matrix <- GOSemSim::mgoSim(term_ids, term_ids, semData = go_data, 
