...
code_text <- 
'
# Extract the beta coefficients for gene 1
betas <- coef(dds)[1, ]

# Define the design matrix row for sample 4
X_j4 <- X[4,]

# Calculate the log2 expected count for gene 1 in sample 4 using the dot product
log2_mu_14 <- sum(betas * X_j4)
log2_mu_14

# Convert the log2 expected count to the original count scale
mu_14 <- 2^log2_mu_14
mu_14 * sizeFactors(dds)[4]
'
...
# Capture the code using substitute
code_lines <- substitute({
  beta_coefficients  <- coef(dds)  # Extract coefficients
  log2_fitted <- beta_coefficients %*% t(X)  # Fit log2
  fitted_counts <- 2^log2_fitted  # Calculate fitted counts
  size_factors <- sizeFactors(dds)  # Get size factors
  normalized_fitted_counts <- sweep(fitted_counts, 2, size_factors, "*")  # Normalize counts
  fitted_counts_deseq <- assays(dds)[["mu"]]  # Extract fitted counts from DESeq2
  result <- all.equal(normalized_fitted_counts, fitted_counts_deseq)  # Compare results
})

# Step 1: Evaluate the captured code to perform calculations
eval(code_lines)  # Perform the calculations

# Step 2: Get the result of the comparison
result_value <- eval(result)  # Evaluate the result expression
...
```{r, results = "asis", echo = FALSE}
# Call the solutions function to display both the code and the result
solutions(code_lines, result_value)
...
The dotplot shows the number of genes (size) associated with the first 10 terms and the p-adjusted values for these terms (color). This plot displays the top 50 genes by gene ratio (# genes related to GO term / total number of significant genes), not the p-adjusted value.
```{r}
## Dotplot of GO terms, let's limit the categories to 20 so we can see the labels
...
# `runGORESP` requires a matrix; we can turn foldchanges into a matrix using `cbind`:
