...
Since tools for differential expression analysis are comparing the counts between sample groups for the same gene, gene length does not need to be accounted for by the tool. However, **sequencing depth** and **RNA composition** do need to be taken into account. The median of ratios method helps neutralize differences in **sequencing depth** and **RNA composition**. DESeq2 normalizes by scaling the counts based on the majority of genes, not influenced by outliers (e.g., extremely highly expressed genes).

On the user-end there is only one step in DESeq2's median of ratios method, but on the back-end there are multiple steps involved, as described below.
...
Where:
- \( x_{1}, x_{2}, \dots, x_{n} \) are the individual numbers in the set,
- \( \prod_{i=1}^{n} x_{i} \) denotes the product of all the numbers.
...
![Alt text](img/deseq_median_of_ratios.png){ width=400 }
...
solutions('PD1_normalized <- PD1 / size_factors 
