...
Formula for geometric mean: multiply all n values of vector x and take the nth root of the product. The advantage of the geometric mean is that it is less affected by very large or very small values.
```{r}
...
geomean <- function(x, na.rm = TRUE){
    ...
    geo = exp(log(prod(x, na.rm = na.rm)^(1/length(x))))
    # which is the same as:
    geo = exp((1/length(x))*sum(log(x), na.rm = na.rm))
    # which is the same as:
...
The colnames of our data don't match the rownames of our metadata so we need to reorder them. We can use the `match` function. Because the order of the columns in the count matrix and the rows in the metadata file don't match, we need to reorder the columns in the count matrix to match the rows in the metadata file. We can use the `match` function to do this. The `match` function returns a vector of the positions of (first) matches of its first argument in its second.
```{r}
